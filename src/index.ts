...
export default class IconSort {
    ...
    setIconConfig(): void {
        ...
        this.coordinates.some((_, index, coordinates) => {
            if (coordinates[index].Y !== coordinates[index + 1].Y) {
                ...
                this.stepY =
                    this.rows > 1
                        ? coordinates[this.cols].Y - coordinates[0].Y
                        : 0
                return true
            }
        })
    ...
    addTouchStart(IconEle: HTMLElement): void {
        IconEle.addEventListener('touchstart', (event) => {
            ...
            if (
                ...
                this.isStatusHold(statusHold.OFF)
            ) {
    ...
    findElementToSwitch(): void {
        ...
        this.dataChanging.targetEle = this.icons[
            this.dataChanging.targetIconIndex
        ]
    }
    elementsSwitch(): void {
      ...
      console.timeEnd("1"); */
            console.time('2')
            const stepEleTransX =
                    this.coordinates[startIndex + index - (forward ? 1 : -1)]
                        .X - this.coordinates[startIndex + index].X,
                stepEleTransY =
                    this.coordinates[startIndex + index - (forward ? 1 : -1)]
                        .Y - this.coordinates[startIndex + index].Y
            console.timeEnd('2')
    ...
    addTransitionend(IconEle: HTMLElement): void {
        ...
        IconEle.addEventListener('transitionend', (event) => {
            clearTimeout(
                this.dataChanging.transitionendDebounce as NodeJS.Timeout
            )
          ...
          this.isStatusHold(statusHold.MOVING) */
                ...
                if (
                    this.isStatus(status.ON) &&
                    this.isStatusHold(statusHold.HOMMING)
                ) {
                    console.log('回归')
                    removeClass(
                        this.dataMoving.grabbedIconEle as HTMLElement,
